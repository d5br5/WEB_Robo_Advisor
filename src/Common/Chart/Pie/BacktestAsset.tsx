...
export const getNameWithTicker = (item: { name: string; ticker: string }) => {
	return `${item.name} ${item.ticker}`;
};

export const getChartLabel = (item: { name: string; ticker: string }) => {
	let rename = item.name;
	if (rename.length > 12) rename = `${rename.substring(0, 12)}...`;
	return `${rename}\n\n${item.ticker}`;
};

const PieChart: FC<{
	label: { name: string; ticker: string }[];
	width: string;
}> = ({ label, width }) => {
	const data = {
		labels: label,
